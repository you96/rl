...
class GenericController extends Controller
{
    ...
    {
    	if ($page != 'contact') $this->_detectUserAgent();
    	$this->_initHost();
    ...
    public function stationAction($name, $permalink)
    {
    	...
    	if(isset($station->body->content->show->animators)){
 		...
 		if (isset($animatorShow[0]) && $animatorShow[0] == false) $animatorShow = null;
		...
		$meta_noindex = '';
		if (empty($station->path[0]->description)){
			$meta_noindex = '<meta name="googlebot" content="noindex">';
		}else{
			if (in_array($station->path[0]->language, array('Français', 'French (Francais)') ) ){
				if ($this->_language != 'fr') $meta_noindex = '<link rel="canonical" href="http://fr-fr.radioline.co'.$_SERVER['REQUEST_URI'].'" />';
				
			}else{
				if ($this->_language != 'en') $meta_noindex = '<link rel="canonical" href="http://www.radioline.co'.$_SERVER['REQUEST_URI'].'" />';
			}
		}
		
		$ogtitle = $station->path[0]->name;
        $singer = '';
    	...
    	'ogtitle'           => ( !empty($_GET['og']) ) ? $ogtitle : $this->get('translator')->trans('Listen to') . ' '.$station->path[0]->name.' ' . $this->get('translator')->trans('radios live') .' - Radio ' . $station->path[0]->name . ' ' . $this->get('translator')->trans('online'),
        'singer'            => $singer,
    	'ogatype'			=> (!empty($_GET['ogadd'])) ? 'music.radio_station' : ''
        ));
    }
	...
	public function podcastAction($name, $permalink)
    {
    	...
    	'presets'		=> $presets,
    	'presets_array'	=> $presets_array,
    	'ogtitle'           => ( !empty($_GET['og']) ) ? $podcast->name : null
        ));
    }
	...
	public function catalogAction($permalink)
    {
    	...
    	if ($this->_connected){
   	 	...
   	 	$aPermalinks = explode('_pp', $permalink);
   	 	$permalink = $aPermalinks[0];
   	 	$page = (isset($aPermalinks[1])) ? $aPermalinks[1] : 1;
    	...
    	$catalog  	= $this->get('radiolineco_webapp.pillow')->getCatalog($this->_country, $this->_language, $this->_locale, $permalink, $page, 24);
    	...
    	if ($this->_connected){
   	 	...
   	 	$divheight = ceil(count($catalog->body->content) / 4) * 160;
   	 	
   	 	$title = $this->get('translator')->trans('Listen to') . ' ' . $this->get('translator')->trans('Radiosh1') . ' ' . $this->get('translator')->trans($catalog->path[count($catalog->path) - 1]->name) . ' ' . $this->get('translator')->trans('radios live');
	...
	public function searchAction($query)
    {
    	...
    	if ($this->_connected){
   	 	...
   	 	$queryArray = explode('_pp', $query);
   	 	$query = $queryArray[0];
   	 	$page = (isset($queryArray[1])) ? $queryArray[1] : 1 ;
   	 	
    	$query = urldecode($query);
    	
    	$stations  	= $this->get('radiolineco_webapp.pillow')->getSearch($this->_country, $this->_language, $this->_locale, $query, $page, 15);
    	...
    	$title = $this->get('translator')->trans('Listen to') . ' radios - '. $query .' '. $this->get('translator')->trans('online');
    	if ($page > 1 ) $title .= ' - Page ' . $page;
    	return $this->render($file, array(
        'page_title' 		=> $title,
    ...
    public function peopleAction($type,$name)
    {
    	$this->initVars('fr');
    	if (!empty($_GET['l'])){
    ...
    public function emissionAction($permalink)
    {
    	...
    	$station		  	= $this->get('radiolineco_webapp.pillow')->getStationInfo($this->_country, $this->_language, $this->_locale, $permalink);
    	$show 				= $this->get('radiolineco_webapp.pillow')->getShowFromStation($this->_country, $this->_language, $this->_locale, $permalink);
    	$showpermalink		= str_replace('podcasts/', '', $show->podcastPermalink);
    	
    	$image 				= $station->body->content->show->logo;
		$description 		= $station->body->content->show->description;
		$podcast  			= $this->get('radiolineco_webapp.pillow')->getPodcastInfo($this->_country, $this->_language, $this->_locale, $showpermalink);
		$chapters 			= $this->get('radiolineco_webapp.pillow')->getPodcastChapters($this->_country, $this->_language, $this->_locale, $showpermalink);
		$similar_podcasts  	= $this->get('radiolineco_webapp.pillow')->getSimilarPodcasts($this->_country, $this->_language, $this->_locale, $showpermalink);
	...
	public function itunessearchAction($singer, $track)
    {
    	$this->initVars('fr');
  	
    	$response = new Response($this->get('radiolineco_webapp.pillow')->getiTunesSearch($this->_country, $this->_language, $this->_locale, $singer, $track));
		//$response->headers->set('Content-Type', 'application/json');
		header("Cache-Control: max-age=30000");
   	 	header("max-age: 30000");
    	return $response;
