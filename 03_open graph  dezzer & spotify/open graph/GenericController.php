...
class GenericController extends Controller
{
    ...
    public function stationAction($name, $permalink)
    {
    	...
    	if(isset($station->body->content->show->animators)){
 		...
 		if ($animatorShow[0] == false) $animatorShow = null;
        ...
        if($name!='radio' && $name.indexOf('from'!=-1)){
            $posi = $name.indexOf('from');
            $ogtitle = $name.substring(0,$posi);
            $singer = $name.substring($posi+4);
        }else{
